...
module.exports = {
  ...
  transformJSRefs: function (block, target, attbs, options) {
    var ref = '';
    ...
    if (block.search(regscript) !== -1) {
      if (attbs) {
        ref = '<script src="' + target + '" ' + attbs + '>';
      } else {
        ref = '<script src="' + target + '">';
      }
    }
    
    var closingTag = '</script>';
    var cdataBlock = '//<![CDATA[ //]]>';
    
    if(options && options.strictXML) {
      ref += cdataBlock;
    }
  ...
  getRef: function (block, blockContent, options) {
    ...
    if (bb.type === 'css') {
      ...
      ref = this.transformJSRefs(blockContent, bb.target, bb.attbs, options);
